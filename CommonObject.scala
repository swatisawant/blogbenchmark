...
/**
  * Created by kishor on 16/11/17.
  */
object CommonObject {

  def getAirlineDataFrame(sqlContext: SQLContext): DataFrame = {
    //val tempAirlineDataFrame: DataFrame = sqlContext.read.format("parquet").load("/home/kishor/SNAPPY/CHECKOUT/snappydata/examples/quickstart/data/airlineParquetData")
    val tempAirlineDataFrame: DataFrame = sqlContext.read.format("parquet").load("/datadrive/users/swati/perfBlog/152/1995-2015_ParquetData")
  ...
  def getAirlineRefDataFrame(sqlContext: SQLContext): DataFrame = {
    //val airportCodeDataFrame: DataFrame = sqlContext.read.format("parquet").load("/home/kishor/SNAPPY/CHECKOUT/snappydata/examples/quickstart/data/airportcodeParquetData")
